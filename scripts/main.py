...
def download_champion_data():
    path = kagglehub.dataset_download('laurenainsleyhaines/25-s1-3-league-of-legends-champion-data-2025')
...
def download_matches_data( gameName:str, tagLine:str, start:int=0, count=20, limit:int=20, match_type:Match_type=Match_type.RANKED, match_region:Match_region=Match_region.EUROPE, puuid_region:Puuid_region=Puuid_region.EUROPE):
    puuid = get_puuid(region=puuid_region, gameName=gameName, tagLine=tagLine)
    match_ids = get_match_ids(region=match_region, type=match_type, puuid=puuid, start=start, count=count, limit=limit, match_ids=[])
    match_results = get_match_results(region=match_region, match_ids=match_ids, puuid=puuid)
...
def evaluate():
    ...
    df['neutralObjectivesTakedowns'] = df['riftHeraldTakedowns'] + df['dragonTakedowns'] + df['voidMonsterKill'] + df['baronTakedowns']
    df['effectiveHealAndShieldingPerMinute'] = df['effectiveHealAndShielding'] / (df['timePlayed'] / 60)
    df['totalTimeCCDealtPerMinute'] = df['totalTimeCCDealt'] / (df['timePlayed'] / 60)
    df['damageTakenPerDeath'] = df['totalDamageTaken'] / np.where(df['deaths'] > 0, df['deaths'], 1)
